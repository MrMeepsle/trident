...
def setup(dataset, root, n_ways, k_shots, q_shots, order, inner_lr, device, download, task_adapt, args):
    if dataset == 'miniimagenet':
        ...
        raise Exception(dataset, "is no known dataset")
...
def loss(reconst_loss: object, reconst_image, image, logits, labels, mu_s, log_var_s, mu_l, log_var_l, reconstr,
         wt_ce=1e2, klwt=False, rec_wt=1e-2, beta_l=1, beta_s=1):
...
def inner_adapt_trident(task, reconst_loss, learner, n_ways, k_shots, q_shots, adapt_steps, device, log_data: bool,
    ...
    eval_loss = loss(reconst_loss, reconst_image, queries,
                     logits, queries_labels, mu_s, log_var_s, mu_l, log_var_l, args.reconstr, args.wt_ce, args.klwt,
                     args.rec_wt, args.beta_l, args.beta_s)
